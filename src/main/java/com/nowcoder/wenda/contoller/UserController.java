...
public class UserController implements WendaConstant {
    ...
    public String uploadHeader(MultipartFile headerImage, Model model) {
        ...
        // 生成随机文件名
        filename = WendaUtil.gennerateUUID() + suffix;
        //确定文件存放的路径
        ...
        String headerUrl = domain + context_path + "/user/header/" + filename;
    ...
    public String changePassword(Model model, String password, String newpassword, @CookieValue("ticket")String ticket){
        if (StringUtils.isBlank(password)){
            model.addAttribute("oriMsg","原始密码不能为空");
            return "/site/setting";
        }
        User user = hostHolder.getUser();
        password = WendaUtil.md5(password + user.getSalt());
        if (!password.equals(user.getPassword())){
            model.addAttribute("oriMsg","原始密码错误，请重新输入！");
            return "/site/setting";
        }
        ...
        password = WendaUtil.md5(newpassword+user.getSalt());
